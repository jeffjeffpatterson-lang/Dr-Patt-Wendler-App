...
const WARMUPS = [{pct: 0.40, reps: 5}, {pct: 0.50, reps: 5}, {pct: 0.60, reps: 3}];
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("wendler531_state");
      if (saved) {
        setState(s => ({ ...s, ...JSON.parse(saved) }));
        setTab("workout");
      }
    } catch (_) {}
    setLoaded(true);
  ...
  function save(newState) {
    ...
    try {
      ...
      localStorage.setItem("wendler531_state", JSON.stringify(toSave));
      setSaveStatus("saved");
  ...
  function resetAll() {
    ...
    try { localStorage.removeItem("wendler531_state"); } catch (_) {}
    setTab("setup");
  }
